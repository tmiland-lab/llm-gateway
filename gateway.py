...
function renderKeys(){
  ...
  api('/api/clients', {action:'list'}).then(({j}) => {
    document.getElementById('clients').innerHTML = '<tr><th>client</th><th>rpm</th><th>req/day (used)</th><th>tok/day (used)</th><th>spent $</th><th>key</th><th></th></tr>' +
      ((j.clients||[]).map(c => '<tr><td>'+esc(c.name)+'</td><td><input type="number" id="rpm-'+esc(c.name)+'" value="'+c.rpm+'" style="width:5em"></td><td><input type="number" id="req-'+esc(c.name)+'" value="'+c.daily_requests+'" style="width:7em"> <span class="mut">('+c.used_req+')</span></td><td><input type="number" id="tok-'+esc(c.name)+'" value="'+c.daily_tokens+'" style="width:8em"> <span class="mut">('+c.used_tok+')</span></td><td>$'+Number(c.spent_usd).toFixed(4)+'</td><td class="mut">'+esc(c.key_hint)+'</td><td><button class="act" onclick="saveClient(\\''+esc(c.name)+'\\')">save</button> <button class="act" onclick="revokeClient(\\''+esc(c.name)+'\\')">revoke</button></td></tr>').join('') || '<tr><td colspan=7 class=mut>none</td></tr>');
  });
}
